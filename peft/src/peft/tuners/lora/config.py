...
@dataclass
class CoSAConfig(LoraConfig):
    """Configuration for the CoSA (Compressed Sensing-based Adaptation) method."""
    
    # You can add any additional parameters specific to your method
    normalize_sum: bool = field(
        default=False,
        metadata={"help": "Whether to normalize the sum of A and B components"}
    )

    use_sum_lora: bool = field(
        default=True,
        metadata={"help": "Whether to use the A+B sum variant instead of A*B multiplication"}
    )


    use_compression: bool = field(
        default=False,
        metadata={"help": "Whether to use compression in the CoSA method"}
    )

    compression_a: int = field(
        default=128,
        metadata={"help": "Compression dimension A for the CoSA method"}
    )

    compression_b: int = field(
        default=128,
        metadata={"help": "Compression dimension B for the CoSA method"}
    )

    use_adaptive_compression: bool = field(
        default=False,
        metadata={"help": "Whether to use adaptive compression based on layer dimensions"}
    )

    layer_compression_config: Optional[Dict[str, Tuple[int, int]]] = field(
        default=None,
        metadata={"help": "Per-layer compression config as {layer_name: (compression_a, compression_b)}"}
    )

    compression_seed: Optional[int] = field(
        default=None,
        metadata={"help": "Seed for generating compression matrices L and R. If None, uses default seed 42."}
    )

    def get_layer_compression_params(self, layer_name: str, in_features: int, out_features: int) -> Tuple[int, int]:
        """
        Get compression parameters for a specific layer based on its dimensions.
        Returns (compression_a, compression_b) tuple.
        """
        if not self.use_adaptive_compression:
            return self.compression_a, self.compression_b
        
        if self.layer_compression_config and layer_name in self.layer_compression_config:
            return self.layer_compression_config[layer_name]
        
        # Dimension-based adaptive strategy
        total_params = in_features * out_features
        
        if total_params > 2048 * 2048:  # Very large layers
            compression_a, compression_b = 2048, 128
        elif total_params > 1024 * 1024:  # Large layers
            compression_a, compression_b = 1024, 128
        elif total_params > 512 * 512:  # Medium layers
            compression_a, compression_b = 1024, 256
        else:  # Small layers
            compression_a, compression_b = 512, 256
        
        # Ensure we don't exceed layer dimensions
        compression_a = min(compression_a, out_features)
        compression_b = min(compression_b, in_features)
        
        return compression_a, compression_b

    def __post_init__(self):
        super().__post_init__()
        self.peft_type = PeftType.COSA