...
from .layer import Conv2d, LoraLayer, CoSAConv2d, CoSAEmbedding, CoSALinear, dispatch_default
...
class CoSAModel(LoraModel):
    """
    CoSA model implementation that uses compressed sensing-based adaptation
    """
    def __init__(self, model, config, adapter_name):
        super().__init__(model, config, adapter_name)
        # Register your custom PEFT type
    ...
    def _create_and_replace(
        ...
        **kwargs
        ...
        """
        Creates and replaces a target module with a CoSA version
        """
        # For layers that already have adapters
        if isinstance(target, BaseTunerLayer):
            target_base_layer = target.get_base_layer()
        else:
            target_base_layer = target
        ...
        if isinstance(target_base_layer, nn.Linear):
            # Get layer-specific compression parameters
            compression_a, compression_b = lora_config.get_layer_compression_params(
                target_name, target_base_layer.in_features, target_base_layer.out_features
            )
            
            new_module = CoSALinear(
                target,
                adapter_name=adapter_name,
                r=lora_config.r,
                lora_alpha=lora_config.lora_alpha,
                lora_dropout=lora_config.lora_dropout,
                fan_in_fan_out=getattr(lora_config, "fan_in_fan_out", False),
                init_lora_weights=lora_config.init_lora_weights,
                use_rslora=getattr(lora_config, "use_rslora", False),
                use_dora=getattr(lora_config, "use_dora", False),
                use_compression=lora_config.use_compression,
                compression_a=compression_a,
                compression_b=compression_b,
                compression_seed=getattr(lora_config, "compression_seed", None),
                **kwargs
            )
            self._replace_module(parent, target_name, new_module, target)
            return new_module
        ...
        # For Embedding layers
            ...
            embedding_kwargs.pop("fan_in_fan_out", None)  # Not used in Embedding
            
            new_module = CoSAEmbedding(
                target,
                adapter_name=adapter_name,
                r=lora_config.r,
                lora_alpha=lora_config.lora_alpha,
        ...
        # For Conv1D layers (from transformers library)
        elif isinstance(target_base_layer, Conv1D):
            is_target_conv_1d = True
            if not kwargs.get("fan_in_fan_out", False):
                kwargs["fan_in_fan_out"] = True  # Conv1D weights are transposed compared to Linear
            ...
            new_module = CoSALinear(
                ...
                use_rslora=getattr(lora_config, "use_rslora", False),
                use_dora=getattr(lora_config, "use_dora", False),
                is_target_conv_1d_layer=is_target_conv_1d,
                **kwargs
            )
            self._replace_module(parent, target_name, new_module, target)
            return new_module
        
        # Default to None if not a supported layer type
        return None