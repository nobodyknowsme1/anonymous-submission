...
def get_peft_model_state_dict(
    ...
    # print(f"config.peft_type: {config.peft_type}")
    if state_dict is None:
        state_dict = model.state_dict()
    # TUNER SPECIFIC CODE
    if config.peft_type in (PeftType.LORA, PeftType.ADALORA):
        # to_return = lora_state_dict(model, bias=model.peft_config.bias)
        # adapted from `[ANONYMOUS_REPO]
    ...
    elif config.peft_type == PeftType.COSA:
        # Only save Y matrices for COSA, not L and R (which are regenerated from seed)
        to_return = {k: state_dict[k] for k in state_dict
                     if "lora_" in k and "lora_L" not in k and "lora_R" not in k}
        # print(f"COSA to_return keys: {to_return.keys()}")
    else:
    ...
    if getattr(model, "modules_to_save", None) is not None:
        for key, value in state_dict.items():
            if any(f"{module_name}.modules_to_save.{adapter_name}" in key for module_name in model.modules_to_save):
                # For COSA, also skip L and R matrices in modules_to_save
                if config.peft_type == PeftType.COSA and ("lora_L" in key or "lora_R" in key):
                    continue
    ...
    # print(f"get_peft_model_state_dict: to_return keys: {to_return.keys()}")
    return to_return
...
def set_peft_model_state_dict(
    ...
    if config.peft_type in (
        ...
        elif config.peft_type == PeftType.LORA or config.peft_type == PeftType.COSA:
            # Here we take care of a refactor of DoRA which changed lora_magnitude_vector from a ParameterDict to a
    ...
    if mismatched_keys:
        ...
        warnings.warn(msg)
    # print(f"set_peft_model_state_dict: load_result: {load_result}")
    return load_result
