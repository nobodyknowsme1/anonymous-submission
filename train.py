...
import transformers
from transformers import Trainer, BitsAndBytesConfig, TrainerState, TrainerControl
from datasets import load_dataset, concatenate_datasets
import datasets
import numpy as np
from peft import LoraConfig, AdaLoraConfig, TaskType, get_peft_model, prepare_model_for_kbit_training, PeftModel, LoraRuntimeConfig
...
def train():
    ...
    # Train on GSM8K/MATH only
    print(f"Total training samples before filtering: {len(raw_train_datasets)}")
    # raw_train_datasets = raw_train_datasets.filter(lambda x: x["type"].lower().startswith("gsm"))
    # raw_train_datasets = raw_train_datasets.filter(lambda x: x["type"].lower().startswith("math"))
    print(f"Total training samples after filtering: {len(raw_train_datasets)}")
